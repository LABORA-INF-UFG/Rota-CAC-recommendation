...
% Colunas: [Greedy, RotaCAC-0.5, RotaCAC-1, RotaCAC-0]
...
fprintf('Iniciando %d execuções para ambos os cenários...\n', num_execucoes);
...
for i = 1:num_execucoes
    ...
    % etas=[0.5,0,1] -> j=1(0.5), j=2(0), j=3(1) -> ordem desejada: [j=1,j=3,j=2]
    resC1_pontos(i,2:4)   = [tmp_p(1), tmp_p(3), tmp_p(2)];
    resC1_qoe(i,2:4)      = [tmp_q(1), tmp_q(3), tmp_q(2)];
    resC1_tamanhos(i,2:4) = [tmp_t(1), tmp_t(3), tmp_t(2)];

    % --- Roda Cenário 2: Com Penalidade (atraso_max = 60) ---
    ...
    % etas=[0.5,0,1] -> j=1(0.5), j=2(0), j=3(1) -> ordem desejada: [j=1,j=3,j=2]
    resC2_pontos(i,2:4)   = [tmp_p(1), tmp_p(3), tmp_p(2)];
    resC2_qoe(i,2:4)      = [tmp_q(1), tmp_q(3), tmp_q(2)];
    resC2_tamanhos(i,2:4) = [tmp_t(1), tmp_t(3), tmp_t(2)];
end
...
figure('Name', 'Análise Comparativa: Impacto da Penalidade', 'Color', 'w', 'Position', [100, 100, 1200, 400]);
